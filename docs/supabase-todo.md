...
**운영 DB에 006과 007을 적용하세요.** (005는 2026-08-12에 적용 완료)

- **006** — 약 복용·병원 방문 표. 2026-08-12 확인 결과 운영 DB에 **아직 없습니다**
  (`medication_records`·`hospital_visits` 둘 다 PostgREST가 PGRST205로 404).
  이것 때문에 세 가지가 함께 막혀 있습니다.
  - '약 · 병원' 화면이 조회부터 실패합니다.
  - 예방접종 화면의 '접종 전 확인 목록'이 비어 있습니다. 최근 체온·투약·병원
    방문을 모아 보여주는 자리인데, 뒤의 둘을 못 읽습니다(코드는 이미 있습니다).
  - 병원 방문을 이유별로 세는 '반복' 카드가 뜨지 않습니다.
- **007** — 커뮤니티 글의 갈래(`posts.category`). 없으면 글쓰기가 CHECK 이전에
  컬럼이 없어 실패합니다.
