...
007까지 운영 DB에 들어가 있습니다. **008·009는 아직 적용 전입니다.**
...
| **008** | **미적용** | `owns_sleep_record`·`owns_temp_record`를 `owns_baby` 경유로. 없으면 함께 키우는 보호자에게 체온 증상과 소음 로그가 막힙니다 |
| **009** | **미적용** | `sleep_noise_stats` 집계 함수. 없으면 소음 통계가 측정 시작 직후만 반영합니다 |
