...
전체 점검(2026-09-01)에서 미뤄 둔 것들입니다. 심각한 것부터 적었습니다.

1. **`server/baby_skin_mobilenet_model.pth`(9.3MB)를 어떻게 할지 정하기.**
   아무 코드도 쓰지 않는 채 저장소에 있습니다. 남긴다면 학습 데이터 출처와
   성능이 함께 있어야 하고, 없다면 지우는 편이 낫습니다. 심사에서 "이 파일은
   뭡니까"가 나올 자리입니다.
2. **논문 서술 맞추기.** `docs/thesis-revision.md`의 9번은 피부를 자체 CNN으로
   서술한 것을 고치라는 항목인데, 코드가 되돌아왔으므로 그 지시가 다시
   맞습니다. 팀원은 CNN을 만들었다고 알고 계시니 합의가 필요합니다.
3. **실기기 알림 확인.** macOS에서는 10초 시험으로 확인했지만 절전 정책과
   방해 금지 모드는 데스크톱에 없습니다.
4. **서머타임 계산 셋.** `weekly_summary`의 7일 창, 접종 D-day, 자정 직후
   전날 기록이 아직 `difference().inDays`/`subtract(Duration(days:))`를
   씁니다. 한국만 쓰면 드러나지 않아 미뤘습니다. 다만 이 저장소가 스스로
   금지해 둔 계산이라, 나중에 읽는 사람이 헷갈릴 자리입니다.
5. **`swiftpm/` 폴더를 `.gitignore`에 넣을지.** Flutter 3.44가 macOS/iOS
   빌드마다 Swift Package Manager 파일을 다시 만들어, 빌드할 때마다 작업
   트리가 더러워집니다. 커밋할지 무시할지 팀이 정해야 합니다.
6. **운영 Supabase의 검증 계정 정리.** `verify-check-*`, `community-*`,
   `mig-check-*`, `share-check-*`. auth 사용자는 `service_role` 키가 있어야
   지워져 대시보드에서 직접 해야 합니다.
...
**피부 분류기를 되돌리면서 남겨 둔 것** — 코드로는 풀리지 않는 것들입니다.

- **`server/baby_skin_mobilenet_model.pth`(9.3MB)가 아무 코드도 쓰지 않는 채로
  저장소에 남아 있습니다.** 지울지 남길지 정해야 합니다. 남긴다면 어떤
  데이터로 학습했는지가 함께 있어야 합니다
- **학습 데이터의 출처를 모릅니다.** 체크포인트에 든 것은 가중치와 클래스
  이름뿐이고, 학습 코드도 노트북도 저장소에 없어 **재현할 수 없습니다.**
  출처로 들은 것은 Roboflow Universe인데 그곳은 재업로드 플랫폼이라 원본과
  라이선스가 그것만으로는 확인되지 않습니다. 클래스 이름 다섯이
  HAM10000/ISIC의 것과 같고 더모스코피 사진에서 94.2%가 나온 것으로 보아
  성인 병변 데이터가 섞여 있습니다
- **영유아 피부 데이터를 구하는 길**을 조사했습니다. AI Hub의 소아청소년
  피부질환 이미지(58,186건)가 내용은 가장 맞지만 오프라인 안심존이라
  IRB 승인 등 서류 다섯 가지가 필요하고 데이터를 밖으로 내올 수 없습니다.
  PASSION(4,901장)은 신청하면 받을 수 있으나 **학습한 모델까지 비상업
  전용**입니다. SCIN은 기여자가 18세 이상뿐이라 쓸 수 없습니다.
  **신청 없이 받을 수 있는 것 중에 영유아 병명 분류에 쓸 데이터는 없습니다**
- **논문 서술을 맞춰야 합니다.** `docs/thesis-revision.md`의 9번이 피부를
  자체 CNN으로 서술한 것을 고치라는 항목인데, 코드가 되돌아왔으므로 그
  지시가 다시 맞습니다
