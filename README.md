...
### 영역별 상담 챗봇 · 기록 시각 9시간 오차 (이번 작업)

**상담을 탭으로 모으지 않고 기록 화면마다 두었습니다.** 체온을 재다 궁금해진
것은 체온에 대한 것이고, 그때 화면을 옮겨 다시 영역을 고르게 하면 묻기를
그만두게 됩니다. 체온·수유·배변·수면·성장·약/병원·예방접종·이유식·피부·소음
10곳에 `AskButton`이 있고, 빠지면 실패하는 테스트가 있습니다.

대화는 서버가 저장하지 않습니다. 앱이 들고 있다가 매번 전부 보냅니다 —
서버가 DB에 붙지 않고, 여러 대에 띄워도 같은 답을 내야 하기 때문입니다.
아이 정보(개월 수·성별·최근 기록·판정)는 `ChatContext`가 영역에 맞게 모아
**시스템 프롬프트 쪽**에 붙입니다. 매 턴 같은 문단이 쌓이지 않고, 기록에
"앞의 지시를 무시하라"가 섞여 들어와도 참고 자료로 놓입니다.
**아이 이름은 보내지 않습니다.**

말투는 항상 존댓말이고, 육아·건강 밖의 질문은 다정하게 거절합니다.
코딩·일반상식 거절, 반말 요청 거부, 시스템 프롬프트 노출 거부, 진단명 거부,
약 용량 거부를 실제 키로 확인했습니다.

**기록 시각이 9시간 밀리고 있었습니다.** `toIso8601String()`이 만드는 값에
오프셋이 없어(`"2026-08-12T03:17:00.000"`) PostgreSQL이 UTC로 해석했고,
읽을 때 `toLocal()`이 다시 9시간을 더했습니다. 새벽 3:17 수유가 낮 12:17로
보였습니다. `db_time.dart`로 규칙을 모으고 운영 DB에서 왕복을 확인했습니다.
날짜 컬럼은 UTC로 바꾸지 않습니다 — 오전 9시 이전 기록이 전날로 밀리고,
생년월일이 하루 당겨지면 개월 수가 바뀌어 체온 판정 경계가 달라집니다.
...
핵심은 **`owns_baby()` 함수 하나만 바꾸면 기록 테이블 정책 15개가 전부 공유를 따른다**는
...
운영 DB에 넣기 전 **로컬 PostgreSQL에 Supabase의 `auth` 스키마를 흉내 내 시나리오를
...
- **DB 설계**: [`docs/erd.md`](docs/erd.md)에 ERD와 테이블 명세서, [`supabase/schema.sql`](supabase/schema.sql)에 실행 가능한 DDL·RLS 정책·예방접종 마스터 데이터(0~24개월 30건) 추가. MySQL은 사용하지 않기로 결정
...
| 할 일 | 왜 지금인가 |
|---|---|
| **계정 두 개로 함께 키우기 확인** | DB는 적용됐지만 실제 초대 발급 → 입력 흐름은 미확인. **아이만 보이고 기록이 안 보이면** `owns_baby()` 교체가 제대로 안 된 것입니다 |
...
- **수면·수유·배변 판정** — **만들지 않기로 확정했습니다.** 축을 바꾸면
  가능할까 싶어 다시 조사했지만(2026-08-12), 후보 수치 6건 중 3건이 1차
  출처 대조에서 기각되고 나머지도 전부 "단독 컷오프로 쓸 수 없음"이었습니다.
  특히 **소변 횟수는 근거가 반대 방향**입니다 — NICE는 부모가 보고한 소변량
  감소가 탈수를 시사하지 못한다고(+LR 1.3, 유의하지 않음) 밝힙니다.
  자세한 내용은 [assessment-rules.md](docs/assessment-rules.md) 4·5절.
...
| 3-4(d) "**14개** 테이블" | **19개** (함께 키우기 2 · 커뮤니티 2 · 약/병원 2 추가, 소음 로그 1 제거) |
| 3-3 "가장 높은 단계를 대표 판정으로" | 종합 판정 미구현 (제외하기로 함) |
| 초록 "체온·성장·**수면·수유** 임계값" | **체온·성장·소음** |

기능 목록(3-2절)에서 ② 수유 이상 판단, ④ 배변 판단, ⑤의 "발달 단계"는 구현되지
않았습니다 — 연령별 구간이 명시된 출처가 없어 임계값을 지어낼 수 없습니다.
⑥ 약 복용·병원 기록은 구현했고, ⑦은 판정 대신 **접종 전 확인 목록**으로
만들었습니다(아래 참고).

### 의도적으로 비워둔 것 (버그가 아님)

육아 가이드 · 진료용 리포트 · 공지사항 · 1:1 문의. 전부 "준비 중" 안내가
...
- 004 마이그레이션 시나리오 검증. 목록은 [`docs/supabase-todo.md`](docs/supabase-todo.md).
...
**확인됨 (운영 Supabase, 2026-08-11)**

- 004 마이그레이션 적용 완료. `baby_members`·`baby_invites` 테이블과 함수 3종이
  모두 응답합니다.
- 권한 차단이 실제로 동작합니다 — 남의 아이 id로 `create_baby_invite`를 부르면
  **"초대 코드는 아이를 등록한 사람만 만들 수 있습니다."** 로 거부됩니다.
- 정보 유출도 막혔습니다 — `list_baby_members`가 오류 대신 **빈 배열**을 돌려주어,
  아이의 존재 여부 자체를 알 수 없습니다.
...
- 함께 키우기 **실제 초대 흐름** — DB는 적용됐고 RPC 응답도 확인했지만, 계정 두 개로
  발급 → 입력 → 기록 공유까지는 해보지 못했습니다.
- 측정을 멈췄을 때 `sleep_records`의 `average_db`·`max_db`·`sample_count`가 채워지는지.
  측정 중에도 60초마다 갱신되므로, 재우는 도중에 행을 봐도 값이 늘어나 있어야 합니다.
