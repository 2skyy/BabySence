...
class Settings:
    ...
    #: 사진 한 장에 대한 토큰 상한. **추론과 본문이 함께 쓰는 상한입니다.**
    #:
    #: 처음에 2048로 두었는데, 애매한 사진일수록 추론이 길어져 상한에 걸립니다.
    #: 걸리면 503이 나가고, 그 503은 이미 횟수 상한 한 칸을 태운 뒤입니다 —
    #: 어려운 사진일수록 실패하고, 안내대로 다시 찍을수록 상한이 줄어드는
    #: 구조였습니다. 상담과 같은 값으로 맞춥니다.
    skin_max_tokens: int = int(os.getenv("SKIN_MAX_TOKENS", 4096))
